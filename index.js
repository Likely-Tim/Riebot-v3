...
database.connect();
database.query(
  "CREATE TABLE IF NOT EXISTS databaseIndex (NAME TEXT PRIMARY KEY, INDEX INT, LENGTH INT);"
);
database.query(
  "CREATE TABLE IF NOT EXISTS messageIndex (NAME TEXT PRIMARY KEY, channelId BIGINT, MESSAGE_ID BIGINT);"
);
database.query(
  "CREATE TABLE IF NOT EXISTS tokens (NAME TEXT PRIMARY KEY, TOKEN TEXT);"
);
database.query(
  "CREATE TABLE IF NOT EXISTS spotifyTrack (INDEX INT PRIMARY KEY, DATA TEXT);"
);
database.query(
  "CREATE TABLE IF NOT EXISTS spotifyArtist (INDEX INT PRIMARY KEY, DATA TEXT);"
);
database.query(
  "CREATE TABLE IF NOT EXISTS spotifyAlbum (INDEX INT PRIMARY KEY, DATA TEXT);"
);

const { spotifyButtonInteraction } = require("./commands/spotify.js");
const {
  spotifyPlayingButtonInteraction,
} = require("./commands/spotify-playing.js");
...
client.on("interactionCreate", async (interaction) => {
  ...
  try {
    const query = await client.commands
      .get(commandName)
      .execute(client, interaction);
    file.prepend(
      "./web/saved/command_log.txt",
      `${user.username}#${user.discriminator},${commandName},${query}\n`
    );
  } catch (error) {
    console.error(error);
...
client.on("messageCreate", async (message) => {
  if (
    ...
    message.author.id == 876282017149505537n
  ) {
    message.delete();
  }
  if (message.author.bot == false) {
    ...
    if (content.startsWith("https://open.spotify.com/track/")) {
      fs.writeFile(
        "./web/saved/spotify.txt",
        content.replace("https://open.spotify.com/track/", "") + "\n",
        { flag: "a+" },
        (err) => {
          if (err) {
            console.log(err);
...
async function reinitializeMessages(client) {
  ...
  for (const key of functionMap.keys()) {
    ...
    const promise = buttonInteractions(
      client,
      results[0],
      results[1],
      functionMap.get(key),
...
async function buttonInteractions(
  client,
  channelId,
  messageId,
  initializer,
