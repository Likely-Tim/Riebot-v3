...
async function sendGetRequestMain(zip) {
  ...
  response = await sendGetRequestForecast(
    response.coord.lat,
    response.coord.lon
  );
...
function startEmbed(response) {
  ...
  result.setTitle(
    `${response.name}, ${titleCap(
      response.weather[0].description
    )}, ${titleDate(response.dt, response.timezone)}`
  );
  result.setDescription(
    `**Current Temperature:** ${currentTemp}\n**Feels Like:** ${feelsLikeTemp}\n**Wind:** ${(
      response.wind.speed * 2.237
    ).toFixed(1)} mph ${windDirection(response.wind.deg)}\n**Humidity:** ${
...
function forecastEmbed(response, result) {
  ...
  if (typeof response.alerts === "undefined") {
    ...
    for (let i = 0; i < response.alerts.length; i++) {
      alerts +=
        response.alerts[i].sender_name +
        ": " +
        response.alerts[i].event +
        " (" +
        dateConvert(response.alerts[i].start, response.timezone_offset) +
        " - " +
        dateConvert(response.alerts[i].end, response.timezone_offset) +
        ")\n";
    }
  }
  const day = new Date(
    (response.daily[1].dt + response.timezone_offset) * 1000
  ).getDay();
  result.addFields(
    {
      name: daysOfTheWeek[day % 7],
      value: `**Min:** ${tempConvert(
        response.daily[1].temp.min
      )}\n**Max:** ${tempConvert(response.daily[1].temp.max)}`,
      inline: true,
    },
    {
      name: daysOfTheWeek[(day + 1) % 7],
      value: `**Min:** ${tempConvert(
        response.daily[2].temp.min
      )}\n**Max:** ${tempConvert(response.daily[2].temp.max)}`,
      inline: true,
    },
    {
      name: daysOfTheWeek[(day + 2) % 7],
      value: `**Min:** ${tempConvert(
        response.daily[3].temp.min
      )}\n**Max:** ${tempConvert(response.daily[3].temp.max)}`,
      inline: true,
    },
    {
      name: daysOfTheWeek[(day + 3) % 7],
      value: `**Min:** ${tempConvert(
        response.daily[4].temp.min
      )}\n**Max:** ${tempConvert(response.daily[4].temp.max)}`,
      inline: true,
    },
    {
      name: daysOfTheWeek[(day + 4) % 7],
      value: `**Min:** ${tempConvert(
        response.daily[5].temp.min
      )}\n**Max:** ${tempConvert(response.daily[5].temp.max)}`,
      inline: true,
    },
    {
      name: "Alerts",
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption((option) =>
      option.setName("zip").setDescription("Enter zip code").setRequired(true)
    ),
