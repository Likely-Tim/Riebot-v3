...
async function responseParse(response, type) {
  ...
  database.query(
    `INSERT INTO databaseIndex VALUES ('spotify${type}', 0, ${length}) ON CONFLICT (name) DO UPDATE SET index = EXCLUDED.index, length = EXCLUDED.length;`
  );
  const current = responseArray[0];
  for (let i = 0; i < length; i++) {
    database.query(
      `INSERT INTO spotify${type} VALUES (${i}, '${responseArray[i]}') ON CONFLICT (index) DO UPDATE SET data = EXCLUDED.data;`
    );
  }
...
async function databaseRetrieve(action, type) {
  let res = await database.query(
    `SELECT * FROM databaseIndex WHERE name = 'spotify${type}';`
  );
  ...
  let content = "";
  database.query(
    `UPDATE databaseIndex SET index = ${index} WHERE name = 'spotify${type}';`
  );
  res = await database.query(
    `SELECT * FROM spotify${type} WHERE index = ${index};`
  );
...
async function disablePreviousMessage(client, newMessage, type) {
  try {
    const res = await database.query(
      `SELECT * FROM messageIndex WHERE name = 'spotify${type}';`
    );
    ...
    await database.query(
      `INSERT INTO messageIndex VALUES ('spotify${type}', ${newMessage.channelId}, ${newMessage.id}) ON CONFLICT (name) DO UPDATE SET channelId = EXCLUDED.channelId, message_id = EXCLUDED.message_id;`
    );
  }
}
...
module.exports = {
  ...
  async execute(client, interaction) {
    ...
    if (result[1] == 0) {
      ...
      components.push(
        button.actionRow(["disabled_prev", "disabled_next", "check"])
      );
    } else {
