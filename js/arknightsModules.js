...
async function refreshArknightsModules() {
    ...
    await page.goto('https://gamepress.gg/arknights/database/arknights-module-list', {waitUntil: 'networkidle2'});
    // Get Module List
    ...
    for (let i = 0; i < naOperators.length; i++) {
        ...
        const operatorName = (await (await info.getProperty("href")).jsonValue()).substring(40);
        const operatorPicture = (await (await (await info.getProperty("firstChild")).getProperty("currentSrc")).jsonValue()).split("?")[0];
